...
def main():
    ...
    # Figure 8: Comparison of State History (Normal vs. DDoS)
    plt.figure(figsize=(14, 7))
    normal_state_history = [0] * 200  # Assuming normal state history remains in state 0
    plt.plot(range(200), normal_state_history, label="Normal State", color='green')
    plt.plot(range(200), state_history[:-1], label="DDoS State", color='blue', linestyle='--')
    plt.xlabel("Time (milliseconds)")
    plt.ylabel("State Value")
    plt.legend()
    plt.title("Figure 8: Comparison of State History (Normal vs. DDoS)")
    plt.grid(True)
    plt.show()
